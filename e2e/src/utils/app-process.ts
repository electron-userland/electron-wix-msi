...
export const launch = async (path: string) => {
  ...
  if (commandArr.length > 2) {
    ...
    const psArgArray = commandArr.splice(2, commandArr.length).join().trim();
  ...
  try {
    ps.addCommand(
      `try { Start-Process ${quotedPath} ${args} -ErrorAction Stop } catch {}`,
    );
    await ps.invoke();
  } finally {
    ps.dispose();
...
export const runs = async (name: string) => {
  ...
  try {
    ps.addCommand(
      `Get-Process ${name.replace(".exe", "")} -ErrorAction SilentlyContinue`,
    );
...
export const getProcessPath = async (name: string) => {
  ...
  try {
    ps.addCommand(
      `$p = Get-Process ${name.replace(".exe", "")} -ErrorAction SilentlyContinue | Select -First 1`,
    );
...
export const kill = async (name: string) => {
  ...
  try {
    ps.addCommand(
      `Get-Process ${name.replace(".exe", "")} -ErrorAction SilentlyContinue | kill -ErrorAction SilentlyContinue`,
    );
    await ps.invoke();
  } finally {
    ps.dispose();
