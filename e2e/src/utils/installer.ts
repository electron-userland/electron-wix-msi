...
function isMSICreatorOptions(
  toBeDetermined: MSICreatorOptions | Options,
): toBeDetermined is MSICreatorOptions {
...
export const install = async (
  msi: string,
  installLevel: 1 | 2 | 3 = 2,
  autoUpdaterUserGroup?: string,
...
export const checkInstall = async (name: string, version?: string) => {
  ...
  try {
    ps.addCommand(
      `$p = Get-Package -Name "${name}" -ErrorAction SilentlyContinue`,
    );
    if (version) {
...
export const getInstallPaths = (
  options: MSICreatorOptions | SquirrelOptions,
  installMode: "perUser" | "perMachine" = "perMachine",
): InstallPaths => {
  ...
  const shortName = isMSICreatorOptions(options)
    ? options.shortName || options.name
    : options.name;
  const genericAumid =
    `com.squirrel.${shortName}.${options.exe!.replace(/\.exe$/, "")}`.toLowerCase();
  const appUserModelId = isMSICreatorOptions(options)
    ? options.appUserModelId?.toLowerCase() || genericAumid
    : genericAumid;
  const toastActivatorClsid = isMSICreatorOptions(options)
    ? options.toastActivatorClsid || emptyGuid
    : emptyGuid;
  const registryRoot = installMode === "perMachine" ? "HKLM" : "HKCU";
  const registryWow =
    arch === "x86" && installMode === "perMachine" ? "WOW6432Node\\" : "";
  const registryRunKey = `${registryRoot}:\\SOFTWARE\\${registryWow}Microsoft\\Windows\\CurrentVersion\\Run`;
  const registryUninstallKey = `${registryRoot}:\\SOFTWARE\\${registryWow}Microsoft\\Windows\\CurrentVersion\\Uninstall`;
  const registryAutoUpdateKey = isMSICreatorOptions(options)
    ? `${registryRoot}:\\SOFTWARE\\${registryWow}\\${options.manufacturer}\\${options.name}`
    : "";
  ...
  const home =
    installMode === "perMachine" ? process.env.Public! : process.env["home"]!;
  return {
    appRootFolder,
    ...
    startMenuShortcut: isMSICreatorOptions(options)
      ? path.join(
          startMenuRoot,
          `${options.shortcutFolderName || options.manufacturer}/${options.shortcutName || options.name}.lnk`,
        )
      : "",
    desktopShortcut: isMSICreatorOptions(options)
      ? path.join(home, `Desktop/${options.shortcutName || options.name}.lnk`)
      : "",
    appUserModelId,
    toastActivatorClsid,
    registryRunKey,
