...
testIncludes(
  "an APPLICATIONROOTDIRECTORY",
  '<Directory Id="APPLICATIONROOTDIRECTORY"',
);

testIncludes(
  "an ApplicationProgramsFolder",
  '<Directory Id="ApplicationProgramsFolder"',
);

testIncludes(
  "a default appUserModelId",
  'Key="System.AppUserModel.ID" Value="com.squirrel.acme.acme"',
);

regexTestIncludes(
  "versioned app folder",
  /<Directory\s*Id=".*"\s*Name="app-1\.0\.0"/,
);

regexTestIncludes(
  "stubbed exe",
  /<File\s*Name="acme\.exe"\s*Id=".*"\s*Source="C:\\Stub\.exe"/,
);
...
test("MSICreator create() sets the appUserModelId", async () => {
  ...
  wxsContent = await fs.readFile(wxsFile, "utf-8");
  expect(
    wxsContent.includes(`Key="System.AppUserModel.ID" Value="Hi"`),
  ).toBeTruthy();
});
...
test("MSICreator compile() throws if candle/light are not installed", async () => {
  ...
  const msiCreator = new MSICreator(defaultOptions);
  expect(msiCreator.compile()).rejects.toEqual(
    new Error("Could not find light.exe or candle.exe"),
  );
});
...
test("MSICreator compile() throws if there is no wxsFile", async () => {
  const msiCreator = new MSICreator(defaultOptions);
  expect(msiCreator.compile()).rejects.toEqual(
    new Error("wxsFile not found. Did you run create() yet?"),
  );
});
...
testIncludes(
  "correct default perUser setting",
  '<Property Id="MSIINSTALLPERUSER" Secure="yes" Value="0" />',
);
testIncludes("Install path property", '<Property Id="INSTALLPATH">');
testIncludes(
  "Install RegistrySearch",
  'RegistrySearch Key="SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall',
);
testIncludes(
  "RegistryInstallPath component",
  '<Component Id="RegistryInstallPath"',
);
testIncludes(
  "RegistryInstallPath component-ref",
  '<ComponentRef Id="RegistryInstallPath" />',
);
testIncludes("PurgeOnUninstall component", '<Component Id="PurgeOnUninstall" ');
testIncludes(
  "PurgeOnUninstall component-ref",
...
testIncludes(
  "UninstallDisplayName component-ref",
  '<ComponentRef Id="UninstallDisplayName" />',
);
testIncludes(
  "UninstallPublisher component-ref",
  '<ComponentRef Id="UninstallPublisher" />',
);
testIncludes(
  "UninstallDisplayVersion component-ref",
  '<ComponentRef Id="UninstallDisplayVersion" />',
);
testIncludes(
  "UninstallModifyString component-ref",
  '<ComponentRef Id="UninstallModifyString" />',
);
testIncludes(
  "UninstallString component-ref",
  '<ComponentRef Id="UninstallString" />',
);
testIncludes(
  "UninstallDisplayIcon component-ref",
  '<ComponentRef Id="UninstallDisplayIcon" />',
);

testIncludesNot("RegistryRunKey component", '<Component Id="RegistryRunKey"');
testIncludesNot(
  "RegistryRunKey component-ref",
  '<ComponentRef Id="RegistryRunKey" />',
);
regexTestIncludesNot(
  "AutoLaunch feature",
  /<Feature Id="AutoLaunch" Title="Launch On Login" Level="2" .*>/,
);
regexTestIncludesNot(
  "AutoUpdate feature",
  /<Feature Id="AutoUpdate" Title="Auto Update" Level="3" .*>/,
);
regexTestIncludesNot(
  "Squirrel executable component-ref",
  /<ComponentRef Id="_msq.exe_.*" \/>/,
);
testIncludesNot(
  "Permission component-ref",
  `<ComponentRef Id="SetFolderPermissions" />`,
);
testIncludesNot(
  "RegistryRunKey component-ref",
  '<ComponentRef Id="SetUninstallDisplayVersionPermissions" />',
);
...
describe("auto-update", () => {
  ...
  regexTestIncludes(
    "Squirrel executable component",
    /<Component Id="_msq.exe_.*"/,
  );
  ...
  testIncludes(
    "SetUninstallDisplayVersionPermissions component-ref",
    '<ComponentRef Id="SetUninstallDisplayVersionPermissions" />',
  );
});
...
describe("auto-launch", () => {
  ...
  testIncludes(
    "RegistryRunKey",
    '<RegistryKey Root="HKMU" Key="SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run" ForceCreateOnInstall="no" ForceDeleteOnUninstall="no"',
  );
  testIncludes(
    "RegistryRunKeyValue",
    '<RegistryValue Name="com.squirrel.acme.acme" Type="string" Value="&quot;[APPLICATIONROOTDIRECTORY]acme.exe&quot;" KeyPath="yes"/>',
  );
  ...
  test("MSICreator includes Auto-Updater feature with arguments", async () => {
    const msiCreator = new MSICreator({
      ...
      features: {
        autoUpdate: false,
        autoLaunch: { enabled: true, arguments: ["arg1", "arg2"] },
      },
    });
    const { wxsFile } = await msiCreator.create();
    wxsContent = await fs.readFile(wxsFile, "utf-8");
  ...
  testIncludes(
    "RegistryRunKey",
    '<RegistryValue Name="com.squirrel.acme.acme" Type="string" Value="&quot;[APPLICATIONROOTDIRECTORY]acme.exe&quot; arg1 arg2" KeyPath="yes"/>',
  );
});
...
describe("perUser install by default", () => {
  ...
  testIncludes(
    "correct defautlt perUser setting",
...
describe("shortcut properties", () => {
  ...
  testIncludes(
    "a default appUserModelId",
    '<ShortcutProperty Key="System.AppUserModel.ID" Value="com.squirrel.acme.acme"',
  );
  testIncludesNot(
    "a ToastActivatorCLSID",
  ...
  testIncludes(
    "a default appUserModelId",
    '<ShortcutProperty Key="System.AppUserModel.ID" Value="com.squirrel.myapp.myapp"/>',
  );
  testIncludes(
    "a ToastActivatorCLSID",
    '<ShortcutProperty Key="System.AppUserModel.ToastActivatorCLSID" Value="{d3519df4-76a7-412f-bd73-9d7746d1d757}"/>',
  );
...
describe("extension association", () => {
  test("MSICreator adds extension associations", async () => {
    const msiCreator = new MSICreator({
      ...defaultOptions,
      associateExtensions: "myext,.myex2",
    });
    const { wxsFile } = await msiCreator.create();
    wxsContent = await fs.readFile(wxsFile, "utf-8");
  ...
  testIncludes(
    "App Description Registry",
    '<RegistryValue Root="HKLM" Key="SOFTWARE\\acme\\Capabilities" Name="ApplicationDescription" Value="Acme"',
  );
  testIncludes(
    "App Exe Registry",
    '<RegistryValue Root="HKLM" Key="SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\acme.exe" Value="[APPLICATIONROOTDIRECTORY]acme.exe"',
  );
  testIncludes(
    "App First Extension Registry Assoc",
    '<RegistryValue Root="HKLM" Key="SOFTWARE\\acme\\Capabilities\\FileAssociations" Name=".myext" Value="acme.myext" Type="string" />',
  );
  testIncludes(
    "App Second Extension Registry Assoc",
    '<RegistryValue Root="HKLM" Key="SOFTWARE\\Classes\\Applications\\acme.exe\\SupportedTypes" Name=".myex2" Value="" Type="string" />',
  );
  testIncludes(
    "Exe file ID in ProgID",
    '<ProgId Id="acme.myext" Description="Acme myext File" Icon="AppIcon.ico',
  );
  regexTestIncludes(
    "Icon included in wxs",
    /<Icon Id="AppIcon.ico" SourceFile=".*Stub.exe"/,
  );
});
