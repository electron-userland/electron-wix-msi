...
import {
  ...
  isChild,
  isDirectChild,
} from "../../src/utils/array-to-tree";
import { separator as S } from "../../src/utils/separator";
import { getMockFileSystem } from "../mocks/mock-fs";

const installInfoRegex =
  process.platform === "win32"
    ? /C:\\tmp\\\.installInfo.*\\\.installInfo\.json/
    : /\/tmp\/\.installInfo.*\.installInfo\.json/;
const msqSquirrelRegex =
  process.platform === "win32" ? /.*\\vendor\\msq.exe/ : /.*\/vendor\/msq.exe/;
...
describe.skip("array-to-tree", () => {
  ...
  test(`addFilesToTree() adds files to a tree structure`, () => {
    const folderFileTree = addFilesToTree(
      mockFolderTree,
      mockFiles,
      mockSpecialFiles,
      mockRegistry,
  ...
  test(`addFilesToTree() adds files to a tree structure`, () => {
    ...
    updaterMockFolderFileTree.__ELECTRON_WIX_MSI_FILES__ = [
      { name: "slack.exe", path: `C:${S}temp${S}slack.exe` },
      { name: ".installInfo.json", path: "C:\\temp\\installInfo.json" },
      { name: "Update.exe", path: "C:\\temp\\Update.exe" },
    ];
